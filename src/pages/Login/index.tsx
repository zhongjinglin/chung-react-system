		...
		<div className="login-container">
			<div className="login-container__form">
				<Tooltip placement="rightTop" title={loginTypes[loginType].tip}>
					<div className={`login-container__form__type ${loginTypes[loginType].className}`} onClick={toggleType}></div>
				</Tooltip>

				<h2 className="login-container__form__title">{loginTypes[loginType].title}</h2>
				<div className="login-container__form__wrap">
					{loginType === 'account' ? <Account /> : <Qrcode setLoginType={setLoginType} />}
				</div>
			</div>
		</div>
	)
